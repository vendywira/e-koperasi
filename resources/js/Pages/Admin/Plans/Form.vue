...
const form = useForm({
    ...
    pricing_config: props.plan?.pricing_config || (props.plan?.type === 'business' ? { price_per_resort: 100000, has_cycle: true } : props.plan?.type === 'enterprise' ? { price: 20000000, has_cycle: false, unlimited: true } : { price: 0, has_cycle: false }),
...
const pricingLabel = computed(() => {
    ...
    switch (form.type) {
        case 'trial': return `Gratis ${form.trial_days} hari, ${form.max_resorts} resort`;
        case 'business': return `Rp${Number(cfg.price_per_resort || 100000).toLocaleString('id-ID')}/resort/bln`;
        case 'enterprise': return `Rp${Number(cfg.price || 0).toLocaleString('id-ID')} (one-time, on-premise)`;
        default: return '-';
    }
});
...
function onTypeChange() {
    switch (form.type) {
        case 'trial':
            form.pricing_config = { price: 0, has_cycle: false };
            form.max_resorts = 1; form.price_per_month = 0; form.trial_days = 30;
            break;
        case 'business':
            form.pricing_config = { price_per_resort: 100000, has_cycle: true };
            form.max_resorts = 1; form.price_per_month = 100000; form.trial_days = 0;
            break;
        case 'enterprise':
            form.pricing_config = { price: 20000000, has_cycle: false, unlimited: true };
            form.max_resorts = 0; form.price_per_month = 20000000; form.trial_days = 0;
            break;
    }
}
